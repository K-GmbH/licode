...
Erizo.DefaultStack = function (spec) {
	"use strict";
	...
	that.peerConnection.onicecandidate =  function (event) {
		if (event.candidate) {
			...
			if (spec.remoteDescriptionSet) {
				spec.callback({type:'candidate', candidate: cloneCandidate});
			} else {
				spec.localCandidates.push(cloneCandidate);
	...
	that.processSignalingMessage = function (msg) {
		...
		if (msg.type === 'offer') {
			...
			var errorHandler = function(text) {
				console.log("setXxxDescription - error:", text);
			};
